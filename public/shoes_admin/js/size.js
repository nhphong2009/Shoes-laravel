...
$(document).ready(function () {
	...
	$('#form-add').submit(function (e) {
		...
		$.ajax({
			...
			success: function (response) {
				toastr.success('Add new size success!');
	...
	//lấy attribute data-url của nút xoá lưu vào url
		var url=$(this).attr('data-url');
		var mytable = $('#sizes-table').DataTable();
		...
		if (confirm("Bạn có chắc chắn muốn xóa không?")){
			$.ajax({
				...
				success: function (response) {
					...
					toastr.success('Delete size success!');
	...
	//bắt sự kiện click vào nút edit
		...
		$.ajax({
			...
			success: function (response) {
				...
				$('#form-edit').attr('data-url',"/admin/size/"+response.data.id)
			},
			error: function (error) {
	...
	$('#form-edit').submit(function (e) {
		...
		$.ajax({
			...
			success: function (response) {
				...
				toastr.success('Edit size success!')
				//ẩn modal edit
