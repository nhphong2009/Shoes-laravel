...
$(document).ready(function () {
	...
	$('#form-add').submit(function (e) {
		...
		$.ajax({
			...
			success: function (response) {
				toastr.success('Add new style success!');
				$('#modal-add').modal('hide')

				$('#styles-table').DataTable().ajax.reload();
	...
	//lấy attribute data-url của nút xoá lưu vào url
		var url=$(this).attr('data-url');
		var mytable = $('#styles-table').DataTable();
		...
		if (confirm("Bạn có chắc chắn muốn xóa không?")){
			$.ajax({
				...
				success: function (response) {
					...
					toastr.success('Delete style success!');
	...
	//bắt sự kiện click vào nút edit
		...
		$.ajax({
			...
			success: function (response) {
				...
				$('#form-edit').attr('data-url',"/admin/style/"+response.data.id)
			},
			error: function (error) {
	...
	$('#form-edit').submit(function (e) {
		...
		$.ajax({
			...
			success: function (response) {
				...
				toastr.success('Edit style success!')
				//ẩn modal edit
